...
- A free [Supabase](https://supabase.com) account — Meltano extracts from a live PostgreSQL source, so this is required, not optional
...
3. Create a service account with the **BigQuery Data Editor** and **BigQuery Job User** roles, and download its JSON key
4. Create two empty BigQuery datasets named **exactly** `olist_raw` and `olist_transformed` — these names are hardcoded throughout the dbt sources, the GE script, and the Dagster assets, so using different names will break the pipeline
...
2. Load the 9 [Olist CSVs](https://www.kaggle.com/datasets/olistbr/brazilian-ecommerce) into PostgreSQL tables in the `public` schema, using the original Kaggle file names as table names
3. Copy `olist_meltano/.env.example` to `olist_meltano/.env` and fill in your Supabase connection string
4. Run:
```bash
cd olist_meltano
